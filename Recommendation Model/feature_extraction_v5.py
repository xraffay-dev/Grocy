...
class SemanticFeatureExtractorV5:
    ...
    def __init__(self):
        ...
        self.brands = [
            # Pakistani brands
            'national', 'shan', 'tapal', 'lipton', 'nestle', 'knorr',
            'dalda', 'habib', 'shezan', 'nurpur', 'olpers', 'walls',
            'lays', 'kolson', 'cocomo', 'sooper', 'peek', 'bisconni',
            'gourmet', 'mitchells', 'rafhan', 'sufi', 'seasons',
            'dawn', 'kausar', 'young', 'english', 'mezan', 'sunrise',
            'qarshi', 'rooh afza', 'pepsi', 'cocacola', 'coca cola', 'sprite',
            'fanta', 'mountain dew', 'aquafina', 'kinley', 'dew',
            
            # Personal care brands
            'lux', 'dove', 'palmolive', 'safeguard', 'dettol', 'lifebuoy',
            'head shoulders', 'pantene', 'sunsilk', 'clear', 'garnier',
            'loreal', 'fair lovely', 'ponds', 'nivea', 'vaseline',
            'axe', 'rexona', 'gillette', 'vince', 'rivaj',
            
            # Cleaning brands
            'surf', 'ariel', 'tide', 'bonux', 'wheel', 'max',
            
            # Oral care
            'colgate', 'close up', 'sensodyne', 'oral b',
            
            # Baby care
            'pampers', 'huggies', 'johnson', 'babylove',
            
            # Food brands
            'maggi', 'kisan', 'heinz', 'shangrila', 'mehran',
            'checkmate', 'candyland', 'hilal', 'polka', 'super crisp',
            'bakea', 'bake parlor', 'rosepair', 'fruitien', 'fresh', 'slice',
            
            # Chocolate brands
            'lindt', 'cadbury', 'kit kat', 'snickers', 'mars', 'twix',
            'dairy milk', 'oreo', 'toblerone',
            
            # Hygiene brands
            'butterfly', 'always', 'whisper', 'carefree', 'stayfree',
            'scotch brite', 'sateen',
        ]
        ...
        self.category_keywords = {
            # Beverages
            'juices': ['juice', 'nectar', 'pulp', 'mango juice', 'orange juice', 'apple juice'],
            'soft_drinks': ['cola', 'sprite', 'fanta', 'pepsi', 'soda', 'fizzy', 'carbonated', 'dew'],
            'water': ['mineral water', 'drinking water', 'aquafina', 'kinley', 'nestle pure'],
            'tea_coffee': ['tea', 'coffee', 'tapal', 'lipton', 'nescafe', 'chai'],
            
            # Dairy
            'milk': ['doodh', 'olpers milk', 'nurpur milk', 'uht milk', 'fresh milk'],
            'dairy_products': ['butter', 'cheese', 'yogurt', 'yoghurt', 'cream', 'ghee', 'dahi'],
            
            # Snacks
            'chocolates': ['chocolate bar', 'choco', 'cocoa'],
            'biscuits': ['biscuit', 'cookie', 'wafer', 'cracker', 'rusk', 'bisconni', 'peek'],
            ...
            'spices': ['spice', 'masala', 'salt', 'pepper', 'chili', 'turmeric', 'shan'],
            'flour_grains': ['flour', 'atta', 'maida', 'rice', 'grain', 'wheat'],
            'ghee': ['ghee', 'desi ghee', 'banaspati'],
            
            # Personal Care
            'hair_care': ['shampoo', 'conditioner', 'hair oil', 'hair color', 'hair dye', 
                         'hair gel', 'hair cream', 'pantene', 'sunsilk', 'head shoulders'],
            'skin_care': ['face wash', 'scrub', 'sunblock', 'sunscreen',
                         'face cream', 'moisturizer', 'ponds', 'nivea', 'fair lovely'],
            'body_care': ['soap', 'body wash', 'shower gel', 'body lotion', 'body spray',
                         'deodorant', 'deo', 'lux', 'dove', 'safeguard', 'lifebuoy'],
            'mens_grooming': ['shaving', 'aftershave', 'razor', 'shave', 'beard', 'axe',
                            'gillette', 'men', 'male'],
            'oral_care': ['toothpaste', 'mouthwash', 'toothbrush', 'dental', 'colgate',
                         'sensodyne', 'close up'],
            
            # Hygiene
            'feminine_hygiene': ['sanitary pad', 'sanitary napkin', 'maternity pad', 
                                'overnight pad', 'tampon'],
            'baby_care': ['diaper', 'pampers', 'huggies', 'baby wipes', 'baby powder',
                         'baby oil', 'baby soap', 'baby lotion'],
            'tissues_wipes': ['tissue', 'wipes', 'towel', 'paper towel'],
            
            # Cleaning
            'laundry': ['detergent', 'washing powder', 'liquid detergent', 
                       'surf', 'ariel', 'tide', 'wheel'],
            'cleaning_supplies': ['cleaner', 'floor cleaner', 'glass cleaner', 
                                 'bathroom cleaner', 'max'],
            
            # Frozen
            'frozen_food': ['chicken', 'meat', 'fish', 'nugget', 'samosa', 'paratha',
                          'sausage', 'frozen', 'k&n'],
            ...
            # Sauces
            'sauces': ['ketchup', 'sauce', 'mayonnaise', 'paste', 'chili sauce',
                      'chilli', 'garlic paste', 'tomato paste', 'kisan', 'heinz'],
        ...
        self.category_hierarchy = {
            ...
            'personal_care': ['hair_care', 'skin_care', 'body_care', 'mens_grooming', 'oral_care'],
    ...
    def extract_basic_features(self, products_df):
        ...
        for idx, row in products_df.iterrows():
            ...
            features = {
                ...
                'avg_word_length': np.mean([len(w) for w in name.split()]) if name.split() else 0,
                'has_numbers': int(bool(re.search(r'\d', name))),
                'number_count': len(re.findall(r'\d+', name)),
                'uppercase_ratio': sum(1 for c in name if c.isupper()) / len(name) if len(name) > 0 else 0,
                ...
                **self.extract_sizes(name),
                ...
                'parent_category': self.get_parent_category(self.infer_category_priority(name)),
                
                # Product type keywords
                **self.extract_product_type_keywords(name),
                
                # Special indicators
                'is_bulk': int(any(x in name.lower() for x in ['pack', 'x', 'dozen', 'family'])),
                'is_organic': int('organic' in name.lower()),
                'is_premium': int(any(x in name.lower() for x in ['premium', 'luxury', 'gold', 'platinum'])),
                'is_lite': int(any(x in name.lower() for x in ['lite', 'light', 'diet', 'low fat'])),
            }
    ...
    def encode_and_scale(self, features_df, semantic_embeddings):
        ...
        parent_category_dummies = pd.get_dummies(df_encoded['parent_category'], prefix='pcat')
        
        print(f"   Created {len(category_dummies.columns)} category features")
        print(f"   Created {len(parent_category_dummies.columns)} parent category features")
        
        # Numerical features
        numerical_features = [
            'name_length', 'word_count', 'char_count', 'avg_word_length',
            'has_numbers', 'number_count', 'uppercase_ratio',
            'price', 'price_log', 'price_sqrt',
            'size_grams', 'size_kg', 'size_ml', 'size_ltr', 'size_pieces',
            'pack_size', 'total_size_normalized',
            'store_encoded', 'brand_encoded',
            'has_oil', 'has_milk', 'has_juice', 'has_water',
            'has_soap', 'has_shampoo', 'has_bread', 'has_rice',
            'has_chicken', 'has_chocolate', 'has_biscuit',
            'has_tea', 'has_coffee', 'has_sauce', 'has_powder', 'has_cream',
            'is_bulk', 'is_organic', 'is_premium', 'is_lite'
        ]
        
        X_numerical = df_encoded[numerical_features].values
        
        # Repeat category features (5x weight for contrastive learning)
        category_features_repeated = np.repeat(category_dummies.values, repeats=5, axis=1)
        parent_category_features_repeated = np.repeat(parent_category_dummies.values, repeats=3, axis=1)
        ...
        # Store feature names
        semantic_feature_names = [f'semantic_{i}' for i in range(semantic_embeddings.shape[1])]
        category_feature_names = [f'{col}_rep{i}' for col in category_dummies.columns for i in range(5)]
        parent_category_feature_names = [f'{col}_rep{i}' for col in parent_category_dummies.columns for i in range(3)]
        
        self.feature_names = numerical_features + semantic_feature_names + category_feature_names + parent_category_feature_names
        ...
        print(f"   Parent category features (weighted): {len(parent_category_feature_names)}")
        print(f"   Total: {X_scaled.shape[1]}")
        print(f"   Semantic weight: {semantic_embeddings.shape[1] / X_scaled.shape[1] * 100:.1f}%")
        print(f"   Category weight: {(len(category_feature_names) + len(parent_category_feature_names)) / X_scaled.shape[1] * 100:.1f}%")
...
def main():
    ...
    try:
        ...
        # Check edge cases
        lindt_products = features_df[features_df['name'].str.contains('Lindt', case=False, na=False)]
        ...
        print("="*70)
        print(f"\n📁 Output files:")
        print(f"   - product_features_v5.csv")
        print(f"   - models/features_scaled_v5.npy")
        print(f"   - models/semantic_embeddings_v5.npy")
        print(f"   - models/feature_extractors_v5.pkl")
        
        print(f"\n🎯 Phase 3 Features:")
        print(f"   ✅ Semantic embeddings: 384 dims")
        print(f"   ✅ Priority-based categories (fixes edge cases)")
        print(f"   ✅ Total features: {X_scaled.shape[1]}")
        print(f"   ✅ Optimized for contrastive learning")
