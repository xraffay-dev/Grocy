...
class ProductMatcher:
    ...
    def find_all_matches(self, product_id: str) -> Dict:
        ...
        semantic_matches = [
            m for m in semantic_matches if m["product"]["productID"] not in exact_ids
        ]

        return {"exact_matches": exact_matches, "semantic_matches": semantic_matches}
...
if __name__ == "__main__":
    ...
    for i, result in enumerate(results[:5], 1):
        ...
        print(
            f"   Type: {result['match_type']}, Confidence: {result['confidence']:.3f}"
        )
